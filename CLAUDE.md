...
### 9.7 Test suite on the iOS 26 simulator — what actually runs (investigated 2026-07-06)
The long-standing "the whole test suite is blocked by a CoreSimulator crash, build-verify only" belief is **wrong and was costing real coverage**. Empirically, on this machine (Xcode 26.3 / iOS 26 sim):
- **Pure-logic / engine tests RUN AND PASS** — `CollectionRollupEngineTests` (5/5), `EngineTests` (streak/weekly-goal, 9/9), `DomainCreateTests`. These create no `ModelContainer`. **Always run them** — never claim "tests blocked" for engine/logic tests (this is the §9.5 gate; it is enforceable, so enforce it). Run just these fast: `-only-testing:HabitsTrackerTests/<EngineSuite> -parallel-testing-enabled NO`.
- **SwiftData `@Model` persistence tests crash the test host at 0.000s** — `CollectionModelTests`, `RuleModelTests`, `ExportImportTests`, `BootstrapBackfillTests`. The crash is in SwiftData creating an in-memory `ModelContainer` for these schemas on the iOS 26 simulator; it is NOT `_findCurrentMigrationStageFromModelChecksum`, NOT a stale on-disk store, and NOT our model code (the app builds and runs fine). Confirmed reproducible clean-sim + isolated + non-parallel; an app-side "skip the container under XCTest" guard was tried and does **not** fix it. **Do not chase these through app code.** Build-verify their compilation; to actually execute them, run on a physical device or a different iOS runtime.
- **`Application failed preflight checks` / `Busy` on host launch** = stale simulator state (often after a bundle-id/team change like `gn.` → `lauterstar.`). Fix with a clean sim: `xcrun simctl shutdown all && xcrun simctl erase <device-id>`, then retry. (Supersedes the old narrower `simctl uninstall <id> lauterstar.HabitsTracker` remedy — erase is more reliable.)

Bottom line for the §9.5 "engine ships with tests" rule: the engine tests are runnable and must be run and reported as passing; only the persistence-layer `@Model` tests are legitimately build-verify-only on this toolchain.
